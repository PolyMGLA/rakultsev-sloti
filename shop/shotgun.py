...
class Shotgun(Gift):
    def __init__(self):
        super().__init__(giftname="🔫пистолет", cost=105, desc="снять у себя и случайного игрока 105🪙")
    ...
    async def open(self, msg: types.Message):
        ...
        if random.randint(1, 100) <= 85 and randu.id != msg.from_user.id:
            if db.add(randu.id, balance=-105):
                logger.info(f"{user.name}({msg.from_user.id}) застрелил {randu.name}({randu.id})")
                await msg.answer(f"Вы застрелили: {randu.name}\nВаш баланс: {db.get(msg.from_user.id, 'balance')}\nЕго баланс: {db.get(randu.id, 'balance')}")
            ...
            if db.add(msg.from_user.id, balance=-105):
                ...
                await msg.answer(f"Вы случайно застрелили себя.\nВаш баланс: {db.get(msg.from_user.id, 'balance')}")
            else:
                await msg.answer("ашипка")
