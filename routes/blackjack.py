...
async def gay_get_card(msg: types.Message, state: FSMContext):
    ...
    if user_score == 21 and db.add(msg.from_user.id, balance=user_sum):
        ...
        if user_score == 21 and db.add(msg.from_user.id, balance=user_sum):
            await msg.answer("Вы победили\n Ваш итоговый счет: 21", reply_markup=blackjack_keyboard)
            await state.clear()
    elif user_score > 21 and db.add(msg.from_user.id, balance=-user_sum):
        await msg.answer("Вы проиграли!", reply_markup=blackjack_keyboard)
        await msg.answer(f"Ваш итоговый счет: {await blackjack.get_sum(state, 'my_cards')}")
        await state.clear()
        ...
        await state.clear()
    elif dealer_score == 87 and db.add(msg.from_user.id, balance=-user_sum):
        ...
        if dealer_score == 21 and db.add(msg.from_user.id, balance=-user_sum):
