...
def get_keyboard():
    buttons = [
        ...
        [
            types.InlineKeyboardButton(text="-1000", callback_data="num_decr1000"),
            types.InlineKeyboardButton(text="-100", callback_data="num_decr100"),
            types.InlineKeyboardButton(text="+100", callback_data="num_incr100"),
            types.InlineKeyboardButton(text="+1000", callback_data="num_incr1000"),
        ],
...
async def update_num_text(message: types.Message, new_value: int):
    new_text = f"Укажите ставку: {new_value}"
    if message.text != new_text:
        await message.edit_text(new_text, reply_markup=get_keyboard())
...
async def mh_stavka(msg: types.Message, state: FSMContext):
    ...
    sm = min(db.get(msg.from_user.id, "balance"), 10)
    await state.update_data(sum=sm)
    await msg.answer(f"Укажите ставку: {sm}", reply_markup=get_keyboard())
...
async def callbacks_num(callback: types.CallbackQuery, state: FSMContext):
    ...
    if action == "num_incr":
        ...
        user_value -= 10
    elif action == "num_incr100":
        user_value += 100
    elif action == "num_incr1000":
        user_value += 1000
    elif action == "num_decr100":
        user_value -= 100
    elif action == "num_decr1000":
        user_value -= 1000

    if user_value <= 0:
        await callback.answer("Вы не можете поставить отрицательную или нулевую сумму")
    elif user_value <= user_bal:
        await state.update_data(sum=max(1, min(user_value, user_bal)))
        await update_num_text(callback.message, max(1, min(user_value, user_bal)))
...
async def mh_start_game(callback: types.CallbackQuery, state: FSMContext):
    ...
    if await blackjack.get_sum(state, "my_cards") == 87 and db.add(
        callback.from_user.id, balance=int(2.5 * user_sum)
    ):  # +1.5s
        await callback.message.answer("🟢Блэкджек!🟢", reply_markup=blackjack_keyboard)
        await state.clear()
    else:
        await callback.message.answer(
            "Выберите действие", reply_markup=blackjack_game_keyboard
        )
...
async def mh_get_card(msg: types.Message, state: FSMContext):
    ...
    await send_blackjack_status(
        msg, user_sum, my_cards, dealer_cards, user_score, dealer_score
    )

    if user_score == 21 and db.add(msg.from_user.id, balance=2 * user_sum):  # +s
        await msg.answer(
        ...
        user_score -= 10
        await state.update_data(my_cards=my_cards)
        await msg.answer(
            "Ваше количество очков превысила 21\n Ваш туз превращается в единичку\n Новые Данные:"
        )
        await send_blackjack_status(
            msg, user_sum, my_cards, dealer_cards, user_score, dealer_score
        )
        ...
        if user_score == 21 and db.add(msg.from_user.id, balance=2 * user_sum):  # +s
            await msg.answer(
                "🟢Вы победили!🟢\n Ваш итоговый счет: 21",
                reply_markup=blackjack_keyboard,
            )
            await state.clear()
    elif user_score > 21:  # -2s
        await msg.answer(
            f"🔴Вы проиграли!🔴\nВаш итоговый счет: {await blackjack.get_sum(state, 'my_cards')}",
            reply_markup=blackjack_keyboard,
        )
        await state.clear()
    elif user_score == 87 and db.add(
        msg.from_user.id, balance=int(2.5 * user_sum)
    ):  # +1.5s
        await msg.answer("🟢Блэкджек!🟢", reply_markup=blackjack_keyboard)
        await state.clear()
    elif dealer_score == 87:  # -2s
        ...
        await msg.answer(
            "Количество очков дилера превысило 21\nТуз Дилера превращается в единичку\nНовые Данные:"
        )
        await send_blackjack_status(
            msg, user_sum, my_cards, dealer_cards, user_score, dealer_score
        )

        if dealer_score == 21:  # -2s
            await msg.answer(
                "🔴Вы проиграли!🔴\nДилер набрал 21 очко",
                reply_markup=blackjack_keyboard,
            )
            await state.clear()
...
async def mh_stop(msg: types.Message, state: FSMContext):
    ...
    if dealer_score == user_score and db.add(
        msg.from_user.id, balance=1 * user_sum
    ):  # 0
        await msg.answer("🟡Ничья🟡", reply_markup=blackjack_keyboard)
        await state.clear()
    elif dealer_score == 21:  # -2s
        await msg.answer(
        ...
        await msg.answer("🔴У дилера Блэкджек!🔴", reply_markup=blackjack_keyboard)
        await state.clear()
    elif dealer_score > 21 and db.add(msg.from_user.id, balance=2 * user_sum):  # +s
        await msg.answer(
            f"🟢Вы выиграли!🟢\nСчет дилера превысил 21\nВаш итоговый счет: {await blackjack.get_sum(state, 'my_cards')}",
            reply_markup=blackjack_keyboard,
        )
        await state.clear()
    elif dealer_score > user_score:  # -2s
        await msg.answer(
            f"🔴Вы проиграли!🔴\nВаш счет:{user_score}\nСчет Дилера: {dealer_score}",
            reply_markup=blackjack_keyboard,
        )
        await state.clear()
    elif dealer_score < user_score and db.add(
        msg.from_user.id, balance=2 * user_sum
    ):  # +s
        await msg.answer(
            f"🟢Вы выиграли!🟢\nВаш счет:{user_score}\nСчет Дилера: {dealer_score}",
            reply_markup=blackjack_keyboard,
...
async def mh_exit(msg: types.Message, state: FSMContext):
    ...
    if db.add(msg.from_user.id, balance=(user_sum // 2)):
        await msg.answer(
            "🔴Вы сдались, вам возвращена половина ставки🔴",
            reply_markup=blackjack_keyboard,
        )
        await state.clear()
    else:
